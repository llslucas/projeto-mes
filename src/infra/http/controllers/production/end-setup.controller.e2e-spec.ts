...
describe("End setup (E2E)", () => {
  ...
  test("[POST] /machines/:machineId/end-setup", async () => {
    ...
    const response = await request(app.getHttpServer())
      .post(`/production/machines/${machine.id.toString()}/end-setup`)
    ...
    expect(reportOnDatabase.type).toBe("Setup end");
  });
});
